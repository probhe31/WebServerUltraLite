...
const showDir = require('./staticLoader/show-dir');
const status = require('./staticLoader/status-handlers');
const generateEtag = require('./staticLoader/etag');
const optsParser = require('./staticLoader/opts');
  ...
  return function middleware(req, res, next) {
    ...
    */
    ...
    if (serverHeader !== false) {
      // Set common headers.
      res.setHeader('server', `staticLoader-WebServerUltraLite`);
    }
...
staticLoader = module.exports;
staticLoader.showDir = showDir;


if (!module.parent) {
  /* eslint-disable global-require */
  /* eslint-disable no-console */
  const defaults = require('./staticLoader/defaults.json');
  const http = require('http');
  const minimist = require('minimist');
  const aliases = require('./staticLoader/aliases.json');
  ...
  if (opts.help || opts.h) {
    console.error('usage: staticLoader [dir] {options} --port PORT');
  } else {
    http.createServer(staticLoader(dir, opts))
      .listen(port, () => {
        console.log(`staticLoader serving ${dir} at http://0.0.0.0:${port}`);
      })
    ;
  }
}
